...
static svfloat32_t cblas_sgemm_sme_load_c(
...
static void cblas_sgemm_sme_kernel(
    int m,
    int n,
    int k,
    float alpha,
...
static void cblas_sgemm_sme_kernel(
    ...
    for (; row + 4 <= m; row += 4) {
        ...
        for (int col = 0; col < n; col += vl) {
            ...
            svfloat32_t c0_vec = cblas_sgemm_sme_load_c(pg, c0, col, beta);
            svfloat32_t c1_vec = cblas_sgemm_sme_load_c(pg, c1, col, beta);
            svfloat32_t c2_vec = cblas_sgemm_sme_load_c(pg, c2, col, beta);
            svfloat32_t c3_vec = cblas_sgemm_sme_load_c(pg, c3, col, beta);
    ...
    for (; row < m; ++row) {
        ...
        for (int col = 0; col < n; col += vl) {
            svbool_t pg = svwhilelt_b32((uint64_t)col, (uint64_t)n);
            svfloat32_t c_vec = cblas_sgemm_sme_load_c(pg, c_row, col, beta);
...
void cblas_sgemm(
    ...
    cblas_sgemm_sme_kernel(m, n, k, alpha, a, lda, b, ldb, beta, c, ldc);
}
